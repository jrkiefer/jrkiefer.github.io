...
- `js/`
  - `config.js` — constants only: `SCRIPT_URL`, `SHEET_URL`, `SIZES`, `BOIL`, `ALL`, `TRAYS_PER_BATCH`, `SIC_MIN`, `SIC_MIN_WAIVER`, `PEACH_MONTHS`, `MAX_BATCH_TEMPS`, `SLOW_DAY_UNDER`, `ROUND_DOWN_MAX_GAP`, `BATCH_DOWN_ALWAYS_OVER`, `BATCH_DOWN_MAX_OVER`, `EXTRA_LG_RATIO`, `BIBLES` (both tables)
...
- Mode visibility is pure CSS off `<html data-mode="twopm"|"eon">` (`.hide-in-eon` / `.hide-in-twopm`). 2 PM and EON have **separate count state** (`counts.js` is instantiated once per mode); both stay hydrated so switching modes is one attribute flip. The tab **always defaults to 2 PM** (v2·15): boot, date change, history open, Load, and Reset each snap to 2 PM (driven from those user-action call sites in main.js, not the store's `load` notify — so the silent resurface reload never yanks the mode on wake). A manual EON tap persists until one of those actions fires. There is no load-time auto-select.
...
- **Day's Work rounding banner** (v2·15, owned by `ui/dayswork.js`): a full-width, colour-coded `#roundBanner` under the hero batch number that spells out `Making {batches×11} vs {totalTrays} planned` plus the split — green **▼ Rounding down** on a cut, amber **▲ Rounding up** on extras, neutral **On the batch — no rounding** when even. `data-dir` (`down`/`up`/`exact`) drives the colour; hidden when the plan isn't ready or tomorrow is closed. It replaced the old cramped `#heroNote` mono line. Derived-only (contract: `update` writes no input values).
...
- `reload({force})` (v2·14) re-pulls the **open** date without blanking state first. Non-force (the resurface auto-refresh) applies the arriving row only when nothing is unsynced (`updatedAt <= syncedAt`); force (the Load button) applies it wholesale. Both keep the phone-only fields (`actualMake`, outlook forecast/manual) from the in-memory record, bail if a `setDate` (`seq`) or a keystroke (`updatedAt`) lands mid-fetch, treat `not_found` as a no-op — never blank a phone because the sheet is empty — and mark a network failure `offline`. A successful apply stamps `updatedAt = syncedAt = now()` and notifies `load` (main.js re-hydrates on that reason). A **force** `not_found` additionally notifies `loadmiss` (v2·15) so main.js can flash a "No saved data on the sheet for …" note under the date card instead of the button looking dead.
...
- **Slow-day rounding** (v2·10, `plan.rounding`): the shared gate is both money forecasts entered and **strictly under $12,000** (`slowDay`; exactly $12k fails). On the gate, the bible lookups default to rounding **down** — per lookup, the tier below is used only when the drop is **≤ $300** (`ROUND_DOWN_MAX_GAP`), else that lookup rounds up; the cap holds even when "down" is stamped manually. **Batch auto-down** (v2·15): batches default to rounding **down** whenever the total is **1–2 trays past a whole batch** (`BATCH_DOWN_ALWAYS_OVER` — 46 trays → 4 batches, on every day regardless of the gate); on a slow day the gate **extends** that window to ≤ 5 trays over (`BATCH_DOWN_MAX_OVER`, remainder 1–5). Manual pills (Bible card = forecast, Day's Work card = batches) stamp `record.forecastRound`/`record.batchRound` and win until Reset; the EON outlook's need lookup follows the same resolved forecast direction (gate always reads the 2 PM forecasts, never the EON-typed one).
...
- **Date matching is timezone-safe** (v2·15): `formatDate` renders Date-typed cells with `Utilities.formatDate` in the **spreadsheet's** timezone, not the script project's. Before the fix, a project/sheet timezone mismatch shifted date-only cells by a day, so `findRowByDate` missed the row, `getByDate` returned `not_found`, and a force-Load looked dead — the "some past days won't load" symptom. `normalizeDate` also canonicalizes ISO (`2026-07-04`) and 2-digit-year (`7/4/26`) cells. **This is the change that actually fixes past-day loading — the pre-v2·15 GET path was identical across v2·11–13, so a version-only redeploy would not have helped.**
...
| v2·15 | From-the-kitchen polish: (1) removed the load-time EON auto-select — the tab **always defaults to 2 PM** on boot/date-change/history-open/Load/Reset, manual EON taps persist until then; (2) **batch auto-down at 1–2 trays over every day** (46 → 4 batches; `BATCH_DOWN_ALWAYS_OVER`), slow days still extend to ≤ 5 over (8 more nights round down across the Apr–Jul export, none the other way); (3) **Day's Work rounding banner** — a colour-coded planned-vs-making callout replacing the cramped `#heroNote`; (4) **past-day loading fix** — timezone-safe `formatDate` + tolerant `normalizeDate` in `Code.gs` (the real cause; the GET path was unchanged since v2·11 so a version-only redeploy wouldn't help), plus a `getByDate` `r.ok` guard and a `loadmiss` note so a force-Load that finds nothing says so | **Yes** |
