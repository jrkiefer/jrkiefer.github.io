...
- `js/`
  ...
  - `calc.js` — pure math, no DOM/fetch/storage: `parseSales`, `money`, `fmt`, `fmtDate`, `countTotal`, `countBlank`, `blankRecord`, `autoBibleFor`, `bibleLookup`, `computePlan`, `effectiveMake`, `computeOutlook` (the slow-day/rounding resolution helpers are internal — read the resolved policy off `plan.rounding`)
  - `api.js` — everything that knows the Apps Script wire format: `post` (text/plain + no-cors retry + keepalive), `getByDate`, `getHistory`, `buildPayloads`, `recordFromRow`, date helpers (`todayISO`, `isoToSheetDate`, `mdyToISO`, `sheetDateToLocal`, `rowToISO`, `toShorthand`); every fetch carries a 15 s `AbortSignal.timeout` (v2·18) so a hung request lands in the offline/retry paths instead of pinning the app
  - `store.js` — record state, synchronous localStorage write-through, debounced background sync, status machine, `reload` re-pull (force/non-force), `cacheHistory` pre-warm, `getLocalRecord` read-only peek (History card's EON sales); `getState` exposes `dirty` (real unsynced edits); **no DOM access**
  ...
  - `main.js` — the only place store and UI meet: view derivation, mode tabs (always open on 2 PM), two-tap reset, date input + one-tap Load (confirm only when dirty), foreground auto-refresh (resurface + 5-min interval + reconnect), boot cache pre-warm, unload flush wiring, boot
- `apps-script/`
  - `Code.gs` — version-controlled copy of the Google Apps Script backend; deploy by manually copying into the Apps Script editor
- `test/` — Node's built-in `node:test`, plain ES-module imports (133 tests)
...
- **Load button** (v2·14, `#loadBtn`, full-width in the date card, wired in main.js): force-pulls the sheet copy of the open date over whatever this phone has — a manual way past local-wins. **One tap when `getState().dirty` is false** (nothing unsynced to lose); when dirty it arms a confirming second tap ("Discard edits & load?", 3 s disarm), since a force-load discards real local edits. The app also **auto-reloads on resurface** (`visibilitychange` → visible and `pageshow`, skipped while `loading` or within 60 s of the last attempt), on a **5-minute interval while open** (v2·18 — a phone on the counter picks up another phone's numbers by itself), and on **reconnect** (v2·18 — the `online` event fires flush *then* reload, so a phone that opened offline recovers the moment the network returns). All non-force, so unsynced local edits always survive; a no-change re-pull is silent (no rehydrate, no 2 PM yank). Since v2·15 a blank/reset date self-heals on the next auto-load, so Load is rarely needed. A force-Load that finds the sheet already matching flashes "Up to date with the sheet." (`loadsame`, v2·18).
...
- `reload({force})` (v2·14) re-pulls the **open** date without blanking state first. It **coordinates with flush** (v2·16): it waits out any in-flight flush (`while (flushRun) await flushRun`) before fetching — the wake sequence fires the `online` flush and the resurface reload together, and Load must show what the sheet holds *after* this phone's own POSTs land — and it **discards** a row fetched while a flush landed mid-GET (`syncedAt` moved since entry — the row may predate the just-synced numbers). Non-force (the resurface/interval/reconnect auto-refresh) applies the arriving row unless the phone has **real (non-blank) unsynced edits** (`hasUnsyncedEdits()`); a blank/reset record yields; force (the Load button) applies it wholesale. A row that **deep-equals the current record** is a quiet no-op (v2·18): sync stamps refresh, status recovers to `synced`, the ack cache is kept (it describes content the sheet provably holds), and nothing notifies — the 5-minute refresh must not rewrite inputs or yank the mode when nothing changed; force notifies `loadsame` instead so main.js flashes "Up to date with the sheet." Both keep the phone-only fields (`actualMake`, outlook forecast/manual) from the in-memory record, bail if a `setDate` (`seq`) or a keystroke (`updatedAt`) lands mid-fetch, and mark a network failure `offline`. A found row **clears the ack cache** on apply (`sent = {}` — a stale ack could swallow a later edit that re-serializes an old payload), stamps `updatedAt = syncedAt = now()`, and notifies `load` (main.js re-hydrates + reverts to 2 PM). A **force** `not_found` restores the status and notifies `loadmiss` (v2·16) so main.js flashes a "No saved data on the sheet for …" note instead of the button looking dead — never blanking a phone because the sheet is empty.
...
- **Reset is two-tap** (armed 2.5 s) and blanks only the open date's record, stamping `updatedAt` with `syncedAt: 0` so the blank never POSTs and overwrites the sheet. It shows blank immediately; the **next** auto-load re-pulls the sheet (v2·15 — a blank has nothing to protect). Sheet rows are NOT deleted — there's no backend API for that.
...
- `npm test` — 133 tests, zero dependencies, Node's built-in `node:test`, plain ES-module imports.
...
- `test/store.test.js` — the sync state machine with mocked fetch/storage and `node:test` mock timers: debounce, ordering, ack cache, offline recovery, mid-flight edits, merge matrix, reset semantics, boot retry, `cacheHistory` gap-fill, the reload rules (force vs non-force, blank-yields-to-sheet, mid-fetch guards, not-found/offline no-ops, `loadmiss`), the reload/flush races (stale-row discard on a mid-fetch sync, wait-for-flush before fetch, cross-date rerun handoff, ack-cache invalidation on apply), and the v2·18 hardening (the gated-empty-flush `flushRun` poison, the reconnect sequence, the clean-record flush guard, the quiet no-change reload incl. `loadsame` and the offline-status recovery).
- `test/codegs.test.js` — Code.gs in a vm context (with `Utilities`/`SpreadsheetApp` timezone stubs): validation branches, **dual bible sync**, Bible + rounding columns, timezone-safe `formatDate` + tolerant `normalizeDate`, seedSheets idempotency (pre-v2 and v2·6-era tabs), the v2·11–13 Dough Use structure (formula rows, stubs, CF rules, dough-save upsert), the reference fit math, formula well-formedness (incl. the New Bible blank-fit guard), and menu registration.
