...
- **One-time data fix**: `fixNegativeMakes()` (in `Code.gs`) is run once from the Apps Script editor. Clamps historical negative values in 2pm Make Amount to 0 and recomputes every Final Dough Amount at 2pm row as Dough Counts + clamped make (rows with no matching Dough Counts row are clamped but their final is skipped). Idempotent — safe to re-run.
...
- **From zero day (closed today)**: Today's Forecast is *optional* — leave it (and Current Sales) empty on a day the shop is closed. Sales Left is then 0, and `computeDough()` uses **zero** dough tonight instead of rounding 0 up to the $3,750 row, so the make equals tomorrow's lookup row exactly and no set-out alert fires. The Bible "Tonight" card shows "—". The save button enables from Tomorrow's Forecast alone (mirroring the backend's `hasForecast` guard). Auto-save still requires all three dollar fields plus counts, so a from-zero save is always a manual tap.
