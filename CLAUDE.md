...
- `test/` — Node's built-in `node:test`, plain ES-module imports (117 tests)
...
- `reload({force})` (v2·14) re-pulls the **open** date without blanking state first. Non-force (the resurface auto-refresh) applies the arriving row unless the phone has real unsynced edits — judged on the sync state **as of entry** (`startUpdatedAt > startSyncedAt && !isBlank`), so a flush that lands mid-fetch can't let a now-stale GET clobber the just-synced edit (v2·15 fix); force (the Load button) applies it wholesale. A blank/reset record yields either way. Both keep the phone-only fields (`actualMake`, outlook forecast/manual) from the in-memory record, bail if a `setDate` (`seq`) or a keystroke (`updatedAt`) lands mid-fetch, treat `not_found` as a no-op — never blank a phone because the sheet is empty — and mark a network failure `offline`. A successful apply stamps `updatedAt = syncedAt = now()` and notifies `load` (main.js re-hydrates + reverts to 2 PM on that reason).
...
- `npm test` — 117 tests, zero dependencies, Node's built-in `node:test`, plain ES-module imports.
...
- `test/store.test.js` — the sync state machine with mocked fetch/storage and `node:test` mock timers: debounce, ordering, ack cache, offline recovery, mid-flight edits, merge matrix, reset semantics, boot retry, `cacheHistory` gap-fill, and the reload rules (force vs non-force, blank-yields-to-sheet, mid-fetch guards, not-found/offline no-ops).
