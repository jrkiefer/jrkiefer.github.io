...
- `js/`
  ...
  - `main.js` — the only place store and UI meet: view derivation, mode tabs (always open on 2 PM), two-tap reset, date input + one-tap Load (confirm only when dirty), foreground auto-refresh, boot cache pre-warm, unload/online flush wiring, boot
- `apps-script/`
  - `Code.gs` — version-controlled copy of the Google Apps Script backend; deploy by manually copying into the Apps Script editor
- `test/` — Node's built-in `node:test`, plain ES-module imports (122 tests)
...
- `flush` is single-flight with a rerun flag; it exposes its in-flight completion as `flushRun` (reload awaits it), and a pass superseded by a mid-flight `setDate` hands off to the queued rerun (v2·16 — `continue`, not `return`) instead of stranding the new date's edits with no timer armed. It builds up to four payloads per record via `api.buildPayloads` and sends them **in order dough → make → temps → eon** (the make correction needs the dough row to exist server-side; it's skipped if dough fails). A per-type serialized-payload ack cache stops unchanged payloads from re-posting. A backend `{status:'error'}` rejection is terminal for that payload version (warned, retried only after the record changes); a network failure sets status `offline` and the payload retries on the next flush/`online` event.
...
- `npm test` — 122 tests, zero dependencies, Node's built-in `node:test`, plain ES-module imports.
...
- `test/store.test.js` — the sync state machine with mocked fetch/storage and `node:test` mock timers: debounce, ordering, ack cache, offline recovery, mid-flight edits, merge matrix, reset semantics, boot retry, `cacheHistory` gap-fill, the reload rules (force vs non-force, blank-yields-to-sheet, mid-fetch guards, not-found/offline no-ops, `loadmiss`), and the reload/flush races (stale-row discard on a mid-fetch sync, wait-for-flush before fetch, cross-date rerun handoff, ack-cache invalidation on apply).
