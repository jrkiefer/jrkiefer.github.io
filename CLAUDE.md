...
- `test/` — Node's built-in `node:test`, plain ES-module imports (118 tests)
...
- `reload({force})` (v2·14) re-pulls the **open** date without blanking state first. Non-force (the resurface auto-refresh) applies the arriving row only when nothing is unsynced (`updatedAt <= syncedAt`); force (the Load button) applies it wholesale. Both keep the phone-only fields (`actualMake`, outlook forecast/manual) from the in-memory record, bail if a `setDate` (`seq`) or a keystroke (`updatedAt`) lands mid-fetch, treat `not_found` as a no-op — never blank a phone because the sheet is empty — and mark a network failure `offline`. Reload also **coordinates with flush** (v2·16): it waits out an in-flight flush before fetching (the wake sequence fires the `online` flush and the resurface reload together, and Load must show what the sheet holds *after* this phone's own POSTs), and discards a row fetched while a flush landed mid-GET (`syncedAt` moved — the row may predate the numbers that just synced). A successful apply stamps `updatedAt = syncedAt = now()`, **clears the per-type ack cache** (a stale ack could swallow a later edit that re-serializes an old payload), and notifies `load` (main.js re-hydrates + re-runs EON auto-select on that reason).
...
- `npm test` — 118 tests, zero dependencies, Node's built-in `node:test`, plain ES-module imports.
...
- `test/store.test.js` — the sync state machine with mocked fetch/storage and `node:test` mock timers: debounce, ordering, ack cache, offline recovery, mid-flight edits, merge matrix, reset semantics, boot retry, and the reload rules (force vs non-force, mid-fetch guards, not-found/offline no-ops, load-after-reset resurrection, the reload/flush races — stale-row discard, wait-for-flush, cross-date rerun handoff, ack-cache invalidation).
...
| v2·16 | Merge-readiness review hardening (full-tree review, July 2026): reload and flush now coordinate — reload waits out an in-flight flush before fetching, discards a row fetched while a flush landed mid-GET (the wake sequence fires the `online` flush and the resurface reload together), and clears the per-type ack cache on apply (a stale ack could silently swallow a re-typed number, leaving the phone "Synced" while the sheet disagreed); a flush superseded by a mid-flight `setDate` re-runs a queued rerun instead of stranding the new date's edits; New Bible tier cells also guard on a blank fit — n ≥ 3 observations with zero sales spread used to spill `#VALUE!` down the whole column where `fitLine` returns blanks. 4 new store tests (118 total) | **Yes + 🍕 Rebuild** (tier formulas live in the sheet; no `seedSheets()`) |
